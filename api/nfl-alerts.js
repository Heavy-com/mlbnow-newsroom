...
async function fetchNocap() {
  try {
    ...
    return body.items.filter((post) => {
      ...
      return leagues.includes('NFL') || matchTeams(text).length > 0;
    });
  } catch (error) {
    return [];
  }
...
module.exports = async (req, res) => {
  ...
  if (!GCHAT_WEBHOOK) return res.status(500).json({ error: 'GCHAT_NFL environment variable not set' });
